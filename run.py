...
from src.core.init import bot, cfg, update_cfg, Log
...
parser.add_argument('--daily-backup', action='store_true', help='Enable daily backup')
...
async def on_ready():
    ...
    log.info(f'Daily backup: {args.daily_backup}')
    update_cfg()
    cortana.init()
...
if args.daily_backup:
    @tasks.loop(time=datetime_time(16, 5))
    async def daily():
        try:
            await Func.daily()
        except Exception as e:
            log.error(f'Daily failed: {e}')
            await warning(f'每日备份失败: {e}', channel=bot.get_channel(cfg['daily']['channel']))
    daily.start()
